...
for ($i=1; $i <=6; $i++) { 
  $s= 'S' .$i;
   if(isset($_POST[$s])){
$sql6="UPDATE etudiant_semestre SET etat='Inscrit' WHERE code_etudiant='$id' AND id_semestre='$i'";
...
foreach ($units as $unit){
  ...
  $sql7 = "UPDATE etudiant_module SET etat = 'Inscrit' WHERE id_module = '$id_module' AND code_etudiant = '$id'; ";
  ...
  if($result == false){
    ...
    $sql6="UPDATE etudiant_semestre SET etat='Non Inscrit' WHERE code_etudiant='$id' AND id_semestre='$i'";
$result6 = mysqli_query($conn, $sql6);
$sql10="SELECT * FROM modules WHERE id_filiere='$_POST[id_filiere]'";
$result = mysqli_query($conn,$sql10);
...
foreach ($units as $unit){
  $id_module = $unit['id_module'];
  $sql7 = "UPDATE etudiant_module SET etat = 'NON INSCRIT' WHERE id_module = '$id_module' AND code_etudiant = '$id'; ";
  $result = mysqli_query($conn,$sql7);
  if($result == false){
    printf("error: %s\n", mysqli_error($conn));
  }	else{
